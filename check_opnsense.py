...
class CheckOPNsense:
    ...
    def check(self) -> None:
        ...
        if self.options.mode == "updates":
            ...
            self.check_ipsec()
        elif self.options.mode == "hastatus":
            self.check_hastatus()
    ...
    def parse_args(self) -> None:
        ...
        check_opts.add_argument(
            ...
            type=str,
            default="",
            help="String that can be use at multible modes to exclude the unwanted from the output / exit code calculation - Example 'Disk 1, Disk 2'"
        )
    ...
    def check_interfaces(self) -> None:
        ...
        if interfaces_down:
            ...
            self.check_message = f"{counter} Interfaces are down\n"
            self.check_message += "\n".join(interfaces_down)
        elif interfaces_up:
            counter = len(interfaces_up)
            self.check_message = f"{counter} Interfaces are up\n"
    ...
    def check_services(self) -> None:
        ...
        if failed_services:
            ...
            self.check_message = f"{counter} Services are failed\n"
            self.check_result = CheckState.CRITICAL
        ...
        for i in failed_services:
            self.check_message += f"[CRITICAL] Service {i} is failed\n"
    ...
    def check_wireguard(self) -> None:
        url = self.get_url(f"wireguard/service/show")
        data = self.request(url)
